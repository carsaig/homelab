...
for variable in MCP_PROXY_BEARER_TOKEN; do
	eval "value=\${${variable}:-}"
	if [ -z "${value}" ]; then
...
echo "Starting mcp-proxy bridge on 127.0.0.1:9090..."
/opt/venv/bin/mcp-proxy --host 127.0.0.1 --port 9090 \
...
echo "MCP bridge ready. Starting Caddy edge on :8080..."
