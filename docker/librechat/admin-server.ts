...
const server = Bun.serve({
  ...
  routes: {
    ...
    '/*': async (req) => {
      ...
      // The session cookie is issued for the mount path, but server functions are called
      // at /_serverFn/... outside it, so a cookie scoped to the mount path never reaches
      // the sign-in exchange. Widen the scope to cover both.
